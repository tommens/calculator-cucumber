...
class MemoryTest {
    ...
    void add() throws IllegalConstruction {
        ...
        assertEquals(m.getMemory().get(0).getValue(), 12);
        assertEquals(m.getMemory().get(0).getExpression(), e);
        assertEquals(l.getMemory().get(0).getValue(), 12);
        assertEquals(l.getMemory().get(0).getExpression(), e);
        List<Expression> params2 = new ArrayList<>();
        Collections.addAll(params2, new MyNumber(3), new MyNumber(3), new MyNumber(3));
        Expression e2 = new Plus(params,Notation.PREFIX);
        m.add("a", 9, e2);
        assertEquals(m.getMemory().get(0).getName(), "a");
        assertEquals(m.getMemory().get(0).getValue(), 9);
        for (int i = 0; i < 98; i++) {
            m.add("a", 9, e2);
        }
        assertThrows(RuntimeException.class, () -> m.add("a", 9, e2));
    }
